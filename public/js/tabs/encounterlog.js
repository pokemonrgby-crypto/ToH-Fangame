...
function setupScrollAnimations() {
    ...
    document.querySelectorAll('.log-paragraph, .dialogue-bubble-wrap').forEach(el => observer.observe(el));
}
...
export async function showEncounterLog() {
    ...
    try {
        ...
        const [charASnap, charBSnap] = await Promise.all([
            fx.getDoc(fx.doc(db, log.a_char)),
            fx.getDoc(fx.doc(db, log.b_char))
        ]);
...
async function render(root, log, charA, charB, logId, comments, myChars) {
    ...
    <section class="container narrow elog-wrap">
      ...
      <div class="elog-comments-wrap">
        ...
        <div id="rating-area" class="kv-card" style="margin-bottom: 1rem;">
          ...
          <div class="row" style="justify-content: space-around; margin-top: 1rem;">
        ...
        <form id="comment-form" class="comment-form" style="${!currentUserId ? 'display:none;' : ''}">
          ...
          <select id="comment-char-select" class="form-control" required>
            <option value="">-- 댓글을 작성할 내 캐릭터 선택 --</option>
    ...
    document.getElementById('btnShare')?.addEventListener('click', () => {
        navigator.clipboard.writeText(location.href).then(()=>showToast('주소가 복사되었습니다!'));
    });
    document.getElementById('btnRematch')?.addEventListener('click', () => {
        location.hash = `#/encounter?cid=${charA.id}&oid=${charB.id}`;
    });
    document.getElementById('btnRelate')?.addEventListener('click', () => {
        location.hash = `#/char/${charA.id}/relate?target=${charB.id}`;
    });

    attachRatingEvents(logId);
    attachCommentEvents(logId);
    attachReportEvents(logId);
}
...
function renderRatingControl(char, side) {
    return `
      <div class="col" style="align-items: center; gap: 8px;">
        ...
        <b>${esc(char.name)}</b>
        <div class="star-rating-half" data-char-id="${char.id}">
          ${[...Array(10)].map((_, i) => {
              const val = (10 - i) / 2;
              const id = `star-${side}-${val}`;
              return `<input type="radio" id="${id}" name="rating-${side}" value="${val}"><label for="${id}">★</label>`;
          }).join('')}
        </div>
      </div>
    `;
...
function renderComment(comment) {
    ...
    <div class="comment-item" data-comment-id="${comment.id}">
      <img src="${esc(comment.photoURL || '')}" class="comment-avatar" onerror="this.style.background='#334155'; this.src='data:image/gif;base64,R0lGODlhAQABAIAAAAAAAP///yH5BAEAAAAALAAAAAABAAEAAAIBRAA7';">
      <div class="comment-body">
        ...
        <div class="comment-meta">
          ${prettyTime(comment.createdAt?.toDate ? comment.createdAt.toDate() : new Date())}
          <button class="btn xs ghost btn-report" style="margin-left: auto;">신고</button>
        </div>
      </div>
    </div>`;
}

function attachRatingEvents(logId) {
    document.querySelectorAll('.star-rating-half input').forEach(radio => {
        radio.addEventListener('change', async (e) => {
            if (!auth.currentUser) return showToast('로그인이 필요합니다.', 'error');
            const rating = parseFloat(e.target.value);
            const charId = e.target.closest('.star-rating-half').dataset.charId;
            const btnContainer = e.target.closest('.star-rating-half');
            btnContainer.style.pointerEvents = 'none'; // 중복 클릭 방지
            try {
                ...
                btnContainer.style.pointerEvents = 'auto'; // 실패 시 다시 활성화
            }
        });
    });
}

function attachCommentEvents(logId) {
    const commentForm = document.getElementById('comment-form');
    if (commentForm) {
        commentForm.onsubmit = async (e) => {
            e.preventDefault();
            const btn = commentForm.querySelector('button[type="submit"]');
            const actingCharId = document.getElementById('comment-char-select').value;
            const rawComment = document.getElementById('comment-text').value;

            if (!actingCharId) { showToast('댓글을 작성할 캐릭터를 선택해주세요.', 'error'); return; }
            if (rawComment.length < 5) { showToast('댓글은 5자 이상 입력해주세요.', 'error'); return; }

            btn.disabled = true;
            btn.textContent = 'AI 변환 중...';
            try {
                const res = await call('commentOnEncounter')({ logId, actingCharId, rawComment });
                if (res.data.ok) {
                    showToast('댓글이 등록되었습니다.');
                    const list = document.getElementById('comments-list');
                    if (list.innerHTML.includes('아직 댓글이 없습니다.')) list.innerHTML = '';
                    // 서버에서 받은 새 댓글 데이터로 렌더링
                    const newCommentHtml = renderComment({ id: res.data.comment.id, ...res.data.comment });
...
function attachReportEvents(logId) {
