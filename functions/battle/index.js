...
평가 기준은 다음과 같습니다:
- **논리성**: 캐릭터의 설정, 배경, 능력 간에 논리적 모순이 없는지 평가합니다. '평범하지만 비범하다'와 같이 상충되는 설명, 자신만의 조건부 승리 등은 극도로 낮은 점수를 부여합니다. 단, 복선 역할을 할만한 모순이나, 분량상 요약이나 생략으로 인한 경우는 페널티를 크게 부여하지 않습니다. 그러나, 강함에는 그 이유가 있어야 하며, 이유나 서사적 상관관계 없이 강함을 묘사한 경우에는 매우 낮은 점수를 부여합니다.
