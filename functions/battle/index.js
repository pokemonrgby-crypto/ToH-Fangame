...
평가 기준은 다음과 같습니다:
- **논리성**: 캐릭터의 설정, 배경, 능력 간에 논리적 모순이 없는지 평가합니다. '평범하지만 비범하다'와 같이 상충되는 설명, 자신만의 조건부 승리, 어떤 조건을 달성하면 승리(예: 상대방이 깨달음을 얻으면 승리, 내가 눈을 감으면 승리) 등은 극도로 낮은 점수를 부여합니다. 단, 복선 역할을 할만한 모순이나, 분량상 요약이나 생략으로 인한 경우는 페널티를 크게 부여하지 않습니다. 그러나, 강함에는 그 이유가 있어야 하며, 이유나 서사적 상관관계 없이 강함을 묘사한 경우에는 매우 낮은 점수를 부여합니다.
...
- **재미성**: 캐릭터 설정이 얼마나 흥미롭고 독창적인지 평가합니다. '무조건 이기는 능력', '조건부 절대 승리', '멍 때림', '뜬금없는 승리' 등 단순하고 일방적인 능력이나, '평범한 회사원'처럼 너무 특징이 없는 설정은 극도로 낮은 점수를 부여합니다. 특히 평범이라는 단어는 해당 캐릭터가 평범하지만 노력한다와 무관하게 캐릭터의 본질이 평범이라는 단어에 빠져 있다면, 또는 평범이 아니더라도 기본적, 흔한 등도 매우 낮은 점수를 부여합니다.
