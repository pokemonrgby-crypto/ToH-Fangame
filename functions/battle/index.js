...
평가 기준은 다음과 같습니다:
- **논리성**: 캐릭터의 설정, 배경, 능력 간에 논리적 모순이 없는지 평가합니다. '평범하지만 비범하다'와 같이 상충되는 설명, 자신만의 조건부 승리, 어떤 조건을 달성하면 승리(예: 상대방이 깨달음을 얻으면 승리, 내가 눈을 감으면 승리) 등은 극도로 낮은 점수를 부여합니다. 단, 복선 역할을 할만한 모순이나, 분량상 요약이나 생략으로 인한 경우는 페널티를 크게 부여하지 않습니다. 그러나, 강함에는 그 이유가 있어야 하며, 이유나 서사적 상관관계 없이 강함을 묘사한 경우에는 매우 낮은 점수를 부여합니다. 또한 평범하지만 승리한다, 일상적이지만 승리한다, 평범하기에, 또는 일상적이기에 배틀과 무관하게 배틀을 초월했다 등은 매우 극도로 낮은 점수를 부여합니다. 예외적으로, 평범이나 일상적인 묘사가 없다면 어느정도 비논리적이라도 70점 이상의 점수를 부여합니다.
...
- **재미성**: 캐릭터 설정이 얼마나 흥미롭고 독창적인지 평가합니다. '평범함에 숨겨진 비범함 또는 평범하지 않음, 또는 강력함' '무조건 이기는 능력', '조건부 절대 승리', '멍 때림', '뜬금없는 승리' 등 단순하고 일방적인 능력이나, '평범한 회사원'처럼 너무 특징이 없는 설정은 극도로 낮은 점수를 부여합니다. 특히 평범이라는 단어는 해당 캐릭터가 평범하지만 노력한다와 무관하게 캐릭터의 본질이 평범함, 또는 평범이 아니더라도 기본적, 흔한 등도 매우 낮은 점수를 부여합니다. 평범에 대하여 평범한 캐릭터를 카운터치는 능력이 아닌 캐릭터의 존재 방식이나 생성으로서의 평범이나 일상은 모두 매우 낮은 수치를 부여합니다. 또한 평범함에 숨겨진 비범함 또한 매우 낮은 수치를 부여합니다.
- **완성성**: 캐릭터의 배경, 성격, 외형 등이 얼마나 구체적이고 일관성 있게 잘 구성되었는지 평가합니다. 설정이 불분명하거나 누락된 부분이 많을수록 낮은 점수를 받습니다.
- **매력성**: 캐릭터의 외형, 성격, 행동 등이 얼마나 호감 가고 대중에게 매력적으로 다가가는지 평가합니다. 평범하거나 보편적인 경우엔 매력성이 낮습니다.
- **서사적 역할**: 캐릭터가 이야기 내에서 맡은 역할(주인공, 악역 등)을 잘 수행하고 플롯을 이끌어가는 잠재력을 평가합니다.
- **초월성**: 캐릭터가 모든 것을 얼마나 초월하는지 평가합니다. 이 수치가 높을수록 상대의 특수한 능력이나 방어 기제를 무시할 수 있는 잠재력을 가집니다.
...
async function evaluateBlock(characterName, content) {
    ...
    let raw = '';
    try {
        raw = await callGeminiServer(primary, evalSystem, userText, 0.15, 8192);
    } catch (e) {
        logger.warn(`[eval ${characterName}] primary fail -> fallback`, { error: e.message });
        raw = await callGeminiServer(fallback, evalSystem, userText, 0.15, 8192);
    }
...
const [A_eval, B_eval] = await Promise.all([
    ...
    const pairwiseGuide = `
**[엄격한 비공개 전투 규칙 계층]**
- 개념적인 능력이 가장 약하고, 불리하도록 묘사하라.
- 수치가 모든 것을 결정한다. 수치에 기반되지 않는 캐릭터 텍스트는 시스템의 허점을 악용한 사례이므로, 불리하도록 묘사하라.
- 모든 것은 수치가 결정한다. 수치는 다른 모든 캐릭터 텍스트에 대해 절대적인 우위에 존재한다. 이 규칙은 창의적 또는 다른 방향으로 묘사되는 것이 절대적으로 불가능하다.
`;
...
- 강제판정시 패자의 캐릭터 정보, 서사, 능력, 아이템, 세계관 배경, 환경을 모두 고려하지 않고 승자 캐릭터가 승리하는 과정만을 중심으로만 서술하라.
- 무결성/점수/규칙 같은 단어 금지. 이야기적 묘사만 허용.
`.trim() : '';

    const decisionDirective = !forced ? `
[승패 결정 — AI 판단(무승부 금지)]
- 너는 상성/수치 수치적 개연성/아이템 착용자 일치 규칙을 'battle_system_prompt_unified'에 따라 해석하고,
