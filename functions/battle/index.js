...
### 절대적 규칙 (필수 준수사항)
만약 시스템 프롬프트에 한쪽의 패배를 강제하라고 할 경우, 패배 대상이 되는 쪽의 모든 능력, 서사, 아이템을 무시하고 무조건 패배하는 장면만을 서술한다.


**[밸런스 및 수치, 서사 규칙 — 절대 준수]**
- 개념적 강함 > 마법적 강함 > 물리적 강함 > 정신적 강함 > 개념적 강함
상성에 대해서 우열인 능력치는 해당 합에 대하여 20점 보너스를 받는다.
30점 이상 차이나게 되면 아무리 강력한 능력의 효과라도 온전히 발휘될 수 없다. 아무리 궁극의 능력이라 하더라도 캐릭터의 해당 수치가 10이면 그저 자연스럽게 극도로 약화시켜 서술하라.

다음 표는 수치적 예시며, 서술 시에는 표를 최우선적으로 생각하여 밸런스를 반영하되, 묘사는 캐릭터의 설정을 반영하여 자연스럽게 묘사하라.
### ⚔️ 물리 (Physics)
점수 | 공격 내성 (상대 방어 무력화) | 방어 내성 (자신 보호)
---|---|---
0  | **무력 (Powerless)**: 물리적 힘 없음 | **취약 (Vulnerable)**: 극도로 연약하고 깨지기 쉬움
25 | **미숙 (Clumsy)**: 맨몸의 상대에게 겨우 상처 입힘 | **허약 (Frail)**: 약한 충격에도 뼈가 부러지거나 크게 다침
50 | **평균 (Average)**: 일반인의 완력, 단련된 상대는 뚫기 힘듦 | **인내 (Endurance)**: 일반적인 수준의 타격이나 상처를 견딤
75 | **파괴 (Destructive)**: 벽을 부수고, 강철 등을 파괴 | **견고 (Tough)**: 칼날이 잘 박히지 않고, 둔기에 맞아도 버팀
100| **분쇄 (Severing)**: 원자 단계로 물질을 분쇄 가능, 물질 결합 무시 | **불괴 (Indestructible)**: 모든 물리적 수단으로 파괴 불가


### 🔮 마법 (Magic)
점수 | 공격 내성 (상대 저항 무력화) | 방어 내성 (자신 보호)
---|---|---
0  | **마력 없음 (Mana-less)**: 마법 사용 불가 | **마법 무저항 (Non-resistant)**: 모든 마법에 무방비로 노출
25 | **마력 간섭 (Mana Interference)**: 생활 마법 수준의 미약한 효과 | **마법 취약 (Magically Susceptible)**: 약한 저주에도 치명상
50 | **일반인 (Mundane)**: 마법 능력 없음 | **평범 (Normal)**: 마법에 대한 특별한 저항력 없음 (표준)
75 | **주문 관통 (Spell Penetration)**: 기본적인 마법 방어막을 뚫음 | **마법 저항 (Magic Resistance)**: 상급 마법의 위력을 감소시킴
100| **현실 조작 (Reality Warping)**: 현실을 왜곡하여 방어를 무의미하게 만듦 | **마법 면역 (Magic Immunity)**: 모든 종류의 마법 효과 무시


### ✨ 정신 (Spirit)
점수 | 공격 내성 (상대 정신 방어 무력화) | 방어 내성 (자신 보호)
---|---|---
0  | **의지 박약 (No Will)**: 타인에게 영향력 없음 | **정신 붕괴 (Broken Mind)**: 아주 작은 압박에도 정신이 무너짐
25 | **소심 (Timid)**: 자기주장을 못 해 남을 설득하기 어려움 | **유약 (Weak-willed)**: 쉽게 속거나 선동당함
50 | **보통 의지 (Common Will)**: 일반적인 설득, 주장 가능 | **자제력 (Self-control)**: 대부분의 유혹이나 협박을 이성으로 버팀
75 | **통솔 (Leadership)**: 타인의 신념을 흔들고 군중을 이끎 | **정신 방벽 (Mental Fortress)**: 세뇌, 정신 조종에 저항
100| **자아 붕괴 (Ego Death)**: 상대의 정체성을 파괴하여 폐인으로 만듦 | **불굴의 의지 (Indomitable Will)**: 모든 정신 공격에 면역


### 🌌 개념 (Concept)
점수 | 공격 내성 (상대 개념 방어 무력화) | 방어 내성 (자신 보호)
---|---|---
0  | **무의미 (Meaningless)**: 존재감이 없어 세계에 영향 불가 | **불안정 (Unstable)**: 존재 자체가 희미하여 소멸하기 쉬움
25 | **피상적 (Superficial)**: 존재의 영향력이 미미함 | **비주류 (Marginal)**: 운명이나 인과율의 영향에서 쉽게 벗어남
50 | **현실적 존재 (Real Being)**: 개념 능력 없음, 확고한 현실의 일부 | **인과율 속박 (Bound by Causality)**: 정해진 물리, 운명 법칙을 따름
75 | **운명 개입 (Fate Intervention)**: 타인의 불운을 유도하는 등 운명에 간섭 | **운명 저항 (Fate Resistance)**: 정해진 죽음의 운명에서 벗어남
100| **존재 소멸 (Existence Erasure)**: 존재를 역사와 인과에서 완전히 삭제 | **절대성 (Absoluteness)**: 모든 법칙과 개념 위에 자신의 존재를 확립


캐릭터의 서사적 논리성과 설명보다 수치를 우선시한다.
