...
        // [BUG FIX] 아이템 차감 로직 수정
        let currentItems = JSON.parse(JSON.stringify(user.items_all || [])); // Deep copy
        for (const itemId in toDeduct) {
            let amountToDeduct = toDeduct[itemId];
            currentItems = currentItems.map(item => {
                if ((item.id === itemId || item.itemId === itemId) && amountToDeduct > 0) {
                    const available = item.count || item.quantity || 0;
                    const canDeduct = Math.min(amountToDeduct, available);
                    item.count = available - canDeduct;
                    amountToDeduct -= canDeduct;
                }
                return item;
            }).filter(item => (item.count || item.quantity || 0) > 0);
        }
...
      const buildingId = uuidv4();
      ...
      // [NEW] 층 복제 로직 추가
      let finalZones = Array.isArray(d.zones) ? JSON.parse(JSON.stringify(d.zones)) : [];
      if (d.replicateLayout && Number(d.floors) > 1) {
          const singleFloorZones = finalZones.slice();
          finalZones = []; // 배열 초기화 후 재구성
          for (let i = 1; i <= Number(d.floors); i++) {
              for (const zone of singleFloorZones) {
                  finalZones.push({
                      ...zone,
                      floor: i // 각 zone에 층 번호 추가
                  });
              }
          }
      }
...
        zones: finalZones, // [MODIFIED] 복제 로직이 적용된 zone 배열 사용
...
      // [BUG FIX] 총면적이 아닌 1층 면적(baseAreaM2)을 더함
      const nextUsedArea = Number(plot.usedArea || 0) + Number(d.baseAreaM2 || 0);
