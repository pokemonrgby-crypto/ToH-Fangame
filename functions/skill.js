...
function pickModels() {
  const shuffled = [...MODEL_OOD_POOL].sort(() => 0.5 - Math.random());
...
module.exports = (admin, { GEMINI_API_KEY }) => {
    ...
    const generateNewSkill = onCall({ region: 'us-central1', secrets: [GEMINI_API_KEY] }, async (req) => {
        ...
        const userPrompt = JSON.stringify({
            character: charData,
            generationMode: generationMode, // 'auto' or 'manual'
        ...
        let aiResult = {};
        try {
            aiResult = await callGemini(GEMINI_API_KEY.value(), systemPrompt, userPrompt, primary);
        } catch (e) {
            logger.warn(`Primary model ${primary} failed, trying fallback ${fallback}`, { error: e.message });
            aiResult = await callGemini(GEMINI_API_KEY.value(), systemPrompt, userPrompt, fallback);
        }
        ...
        // 생성된 스킬을 즉시 저장하지 않고 클라이언트로 반환하여 확인받습니다.
    ...
    const confirmAddSkill = onCall({ region: 'us-central1' }, async (req) => {
        ...
        return await db.runTransaction(async (tx) => {
            ...
            tx.update(userRef, {
                ...
                lastSkillCreatedAt: FieldValue.serverTimestamp()
            });

            tx.update(charRef, {
                abilities_all: FieldValue.arrayUnion(skill)
            });
