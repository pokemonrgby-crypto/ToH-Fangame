...
module.exports = (admin, { GEMINI_API_KEY }) => {
    ...
    const startOrGetStoryRunV4 = onCall({ region: 'us-central1' }, async (req) => {
        ...
        const initialHp = rules.leveling.hpBase + (rules.leveling.hpPerLevel * (calculateLevel(charSnap.data()[rules.leveling.expField] || 0) - 1));
        ...
        const playerState = {
            ...
            hp: initialHp,
            maxHp: initialHp,
            level: calculateLevel(charSnap.data()[rules.leveling.expField] || 0),
    ...
    const progressBattleTurnV4 = onCall({ region: 'us-central1', secrets: [GEMINI_API_KEY] }, async (req) => {
        ...
        const playerLevel = calculateLevel(character[rules.leveling.expField] || 0);

        // --- 2. 서버사이드 판정 (막기 확률) ---
        const blockBase = rules.blockBase[monster.difficulty][monster.grade];
        const levelDiff = playerLevel - (monster.level || (DIFFICULTIES.indexOf(monster.difficulty) * 10 + 5));
...
JSON 출력 예시:
{
  ...
  "damageToPlayer": 0
        ...
        const aiResult = await callGemini(GEMINI_API_KEY.value(), 'gemini-1.0-pro', system, user);

        // --- 4. 결과 처리 및 상태 업데이트 (트랜잭션) ---
        return await db.runTransaction(async (tx) => {
            ...
            mState.hp -= aiResult.damageToMonster || 0;
            pState.hp -= aiResult.damageToPlayer || 0;

            const battleLog = [
                ...pState.battle.log,
                `[턴 ${pState.battle.turn}] ${aiResult.narrative}`
            ];
            ...
            if (outcome) {
                ...
                if (outcome === 'win') {
                    ...
                    const expGain = (rules.ENEMY_GRADES.indexOf(mState.grade) + 1) * 10 * (rules.DIFFICULTIES.indexOf(mState.difficulty) + 1);
                ...
                return { ok: true, outcome, battleLog, rewards };
                ...
                tx.update(playerRef, { ...pState, updatedAt: Timestamp.now() });
                return { ok: true, outcome: 'ongoing', battleLog };
            }
        });
    });
